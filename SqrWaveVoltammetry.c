...
/* SDK compatibility shims (different AD5940 library revisions use different names) */
#ifndef DPV_ADCMUXP_LPTIA
  #if defined(ADCMUXP_LPTIA0_P)
    #define DPV_ADCMUXP_LPTIA ADCMUXP_LPTIA0_P
  #elif defined(ADCMUXP_LPTIA_P)
    #define DPV_ADCMUXP_LPTIA ADCMUXP_LPTIA_P
  #elif defined(ADCMUXP_VCE0)
    #define DPV_ADCMUXP_LPTIA ADCMUXP_VCE0
  #else
    #error "No compatible ADC positive MUX selection symbol found for DPV."
  #endif
#endif

#ifndef DPV_ADCMUXN_LPTIA
  #if defined(ADCMUXN_LPTIA0_N)
    #define DPV_ADCMUXN_LPTIA ADCMUXN_LPTIA0_N
  #elif defined(ADCMUXN_LPTIA_N)
    #define DPV_ADCMUXN_LPTIA ADCMUXN_LPTIA_N
  #elif defined(ADCMUXN_VSET0)
    #define DPV_ADCMUXN_LPTIA ADCMUXN_VSET0
  #elif defined(ADCMUXN_VZERO0)
    #define DPV_ADCMUXN_LPTIA ADCMUXN_VZERO0
  #elif defined(ADCMUXN_AIN4)
    #define DPV_ADCMUXN_LPTIA ADCMUXN_AIN4
  #else
    #error "No compatible ADC negative MUX selection symbol found for DPV."
  #endif
#endif

#ifndef DPV_AFE_PWRMASK
  #define DPV_AFE_PWRMASK AFECTRL_HPREFPWR
  #ifdef AFECTRL_HPTIAPWR
    #undef DPV_AFE_PWRMASK
    #define DPV_AFE_PWRMASK (AFECTRL_HPREFPWR|AFECTRL_HPTIAPWR)
  #endif
  #ifdef AFECTRL_LPREFPWR
    #undef DPV_AFE_PWRMASK
    #ifdef AFECTRL_HPTIAPWR
      #define DPV_AFE_PWRMASK (AFECTRL_HPREFPWR|AFECTRL_HPTIAPWR|AFECTRL_LPREFPWR)
    #else
      #define DPV_AFE_PWRMASK (AFECTRL_HPREFPWR|AFECTRL_LPREFPWR)
    #endif
  #endif
  #ifdef AFECTRL_LPTIAPWR
    #undef DPV_AFE_PWRMASK
    #if defined(AFECTRL_HPTIAPWR) && defined(AFECTRL_LPREFPWR)
      #define DPV_AFE_PWRMASK (AFECTRL_HPREFPWR|AFECTRL_HPTIAPWR|AFECTRL_LPREFPWR|AFECTRL_LPTIAPWR)
    #elif defined(AFECTRL_HPTIAPWR)
      #define DPV_AFE_PWRMASK (AFECTRL_HPREFPWR|AFECTRL_HPTIAPWR|AFECTRL_LPTIAPWR)
    #elif defined(AFECTRL_LPREFPWR)
      #define DPV_AFE_PWRMASK (AFECTRL_HPREFPWR|AFECTRL_LPREFPWR|AFECTRL_LPTIAPWR)
    #else
      #define DPV_AFE_PWRMASK (AFECTRL_HPREFPWR|AFECTRL_LPTIAPWR)
    #endif
  #endif
#endif
...
static AD5940Err AppDPVSeqInitGen(void)
{
  ...
  /* DSP config (SINC3) */
  /* Route ADC directly to LPTIA output when symbols are available. */
  dsp_cfg.ADCBaseCfg.ADCMuxP = DPV_ADCMUXP_LPTIA;
  dsp_cfg.ADCBaseCfg.ADCMuxN = DPV_ADCMUXN_LPTIA;
  ...
  AD5940_AFECtrlS(DPV_AFE_PWRMASK, bTRUE);
...
static AD5940Err DPVDacRegUpdate(uint32_t *pDACData, uint32_t stepIndex, BoolFlag pulsePhase)
{
  uint32_t VzeroCode, VbiasCode;
  float Ebase_mV;
  float Vzero_mV;

  if(stepIndex >= AppDPVCfg.StepNumber)
    return AD5940ERR_APPERROR;

  Vzero_mV = (stepIndex < (AppDPVCfg.StepNumber/2)) ? AppDPVCfg.VzeroStart : AppDPVCfg.VzeroPeak;

  Ebase_mV = AppDPVCfg.RampStartVolt + (float)stepIndex * AppDPVCfg.StaircaseInc;
  if(Ebase_mV > AppDPVCfg.RampPeakVolt)
    Ebase_mV = AppDPVCfg.RampPeakVolt;

  AppDPVCfg.CurrBaseCode = Ebase_mV / DAC12BITVOLT_1LSB;
  VzeroCode = (uint32_t)((Vzero_mV - 200.0f)/DAC6BITVOLT_1LSB);

  {
    ...
    float combined = (float)VzeroCode * 64.0f + vbias12;

    if(combined < 0.0f) combined = 0.0f;
    if(combined > 4095.0f) combined = 4095.0f;
    VbiasCode = (uint32_t)combined;
  }

  if(VzeroCode > 63) VzeroCode = 63;
  AppDPVCfg.CurrVzeroCode = VzeroCode;
...
static AD5940Err AppDPVSeqDACCtrlGen(void)
{
  ...
  {
    uint32_t *pStepPos = genSeq0 ? &AppDPVCfg.CurrStepPos : &AppDPVCfg.PulseStepPos;
    uint32_t stepsLeft = AppDPVCfg.StepNumber - (*pStepPos);
  ...
  if(StepsThisBlock == 0)
  {
    if(AppDPVCfg.CurrStepPos >= AppDPVCfg.StepNumber && AppDPVCfg.PulseStepPos >= AppDPVCfg.StepNumber)
  ...
  {
    uint32_t stepIndex = (genSeq0 ? AppDPVCfg.CurrStepPos : AppDPVCfg.PulseStepPos) + i;
    error = DPVDacRegUpdate(&DACData, stepIndex, genSeq0 ? bFALSE : bTRUE);
  ...
  if(genSeq0)
    AppDPVCfg.CurrStepPos += StepsThisBlock;
  else
    AppDPVCfg.PulseStepPos += StepsThisBlock;
...
/* ---------- Data process: convert FIFO samples into ?I pairs ---------- */
static int32_t AppDPVDataProcess(int32_t * const pData, uint32_t *pDataCount)
{
  ...
  /* Collapse into ?I pairs: (pulse - before) */
  uint32_t pairs = n/2;
...
AD5940Err AppDPVISR(void *pBuff, uint32_t *pCount)
{
  ...
  if(IntFlag & AFEINTSRC_DATAFIFOTHRESH)
  {
    ...
    *pCount = count;
    ...
    if(AppDPVCfg.StopRequired == bTRUE || AppDPVCfg.RampState == DPV_STOP)
    {
      AppDPVCtrl(APPCTRL_STOPNOW, 0);
      AppDPVCfg.StopRequired = bFALSE;
    }
