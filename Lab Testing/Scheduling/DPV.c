/* ============================= DPV.c =============================
   ...
   - MODIFIED: peak extraction only (calibration/concentration removed)
================================================================== */
...
static float gDPV_GlobalPeak_uA = 0.0f;

/* Expose latest peak value */
  ...
  *pCount = 0;
  ...
  if(IntFlag & AFEINTSRC_DATAFIFOTHRESH)
  {
    ...
    uint32_t nOut = AppDPVDiffProcess((float*)pBuff, FifoCnt);
		...
		float peakChunk = DPV_FindPeakAbs_uA((float*)pBuff, nOut);

			if(peakChunk > gDPV_GlobalPeak_uA)
				gDPV_GlobalPeak_uA = peakChunk;
  ...
  if(IntFlag & AFEINTSRC_ENDSEQ)
  {
    ...
    /* Final chunk update */
		...
		/* === FINAL PEAK OVER FULL SCAN === */
		gDPV_LastPeak_uA = gDPV_GlobalPeak_uA;
