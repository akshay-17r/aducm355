...
void AD5940DPVStructInit(void)
{
  ...
  pCfg->RcalVal = 200.0f;        /* RCAL on EVB (ohms) */
  pCfg->ADCRefVolt = 1820.0f;    /* mV */
  pCfg->FifoThresh = 480;
  ...
  pCfg->DPV_PulseHeight = 50.0f;    /* mV */

  pCfg->StaircaseInc = 5.0f;        /* mV */
  pCfg->SampleDelay = 10.0f;        /* ms */

  pCfg->LPTIARtiaSel = LPTIARTIA_8K; /* internal RTIA selection */
