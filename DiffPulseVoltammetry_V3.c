...
#ifndef DPV_ADCMUXN_FALLBACK
  #if defined(ADCMUXN_VSET0)
    #define DPV_ADCMUXN_FALLBACK ADCMUXN_VSET0
  #elif defined(ADCMUXN_VZERO0)
    #define DPV_ADCMUXN_FALLBACK ADCMUXN_VZERO0
  #elif defined(ADCMUXN_AIN4)
    #define DPV_ADCMUXN_FALLBACK ADCMUXN_AIN4
  #else
...
/* ---------- Calibrate LP TIA RTIA and use measured value ---------- */
static AD5940Err AppDPVRtiaCal(uint8_t lpAmpSel)
{
  AD5940Err err;
  ...
  lprtia_cal.fFreq = AppDPVCfg.SysClkFreq/4.0f/22.0f/2048.0f*3.0f;
  ...
  err = AD5940_LPRtiaCal(&lprtia_cal, &rtiaCalValue);
  if(err != AD5940ERR_OK)
    return err;

  if(rtiaCalValue.Magnitude <= 0.0f)
...
static AD5940Err AppDPVSeqInitGen(void)
{
  ...
  dsp_cfg.ADCBaseCfg.ADCMuxN = DPV_ADCMUXN_FALLBACK;
...
AD5940Err AppDPVInit(uint32_t *pBuffer, uint32_t BufferSize)
{
  ...
  /* Use measured RTIA instead of nominal selection values. */
  error = AppDPVRtiaCal(LPAMP0);
  if(error != AD5940ERR_OK) return error;

  /* Basic FIFO setup (configure after calibration because calibration routine reconfigures AFE/DSP). */
  ...
  /* Generate sequences after calibration to restore DPV runtime AFE configuration. */
